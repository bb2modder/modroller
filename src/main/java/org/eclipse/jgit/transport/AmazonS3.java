...
public class AmazonS3 {
	...
	private final org.eclipse.jgit.transport.WalkEncryption encryption;
	...
	public AmazonS3(final Properties props) {
		...
		try {
			encryption = org.eclipse.jgit.transport.WalkEncryption.instance(props);
	...
	public void put(String bucket, String key, byte[] data)
			throws IOException {
		if (encryption != org.eclipse.jgit.transport.WalkEncryption.NONE) {
	...
	HttpURLConnection open(final String method, final String bucket,
		...
		c.setRequestProperty("User-Agent", "org/eclipse/jgit/1.0"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private final class ListParser extends DefaultHandler {
		...
		void list() throws IOException {
			...
			for (int curAttempt = 0; curAttempt < maxAttempts; curAttempt++) {
				...
				default:
					throw org.eclipse.jgit.transport.AmazonS3.this.error("Listing", prefix, c); //$NON-NLS-1$
				}
			}

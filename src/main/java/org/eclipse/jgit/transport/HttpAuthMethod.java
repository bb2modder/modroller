...
abstract class HttpAuthMethod {
	...
	public enum Type {
		NONE {
			@Override
			public org.eclipse.jgit.transport.HttpAuthMethod method(String hdr) {
		 ...
		 */
		public abstract org.eclipse.jgit.transport.HttpAuthMethod method(String hdr);
	 ...
	 */
	static org.eclipse.jgit.transport.HttpAuthMethod scanResponse(final HttpConnection conn,
	                                                              Collection<Type> ignoreTypes) {
		final Map<String, List<String>> headers = conn.getHeaderFields();
		org.eclipse.jgit.transport.HttpAuthMethod authentication = Type.NONE.method(EMPTY_STRING);
	...
	private static class None extends org.eclipse.jgit.transport.HttpAuthMethod {
	...
	private static class Basic extends org.eclipse.jgit.transport.HttpAuthMethod {
		private String user;
		...
		void configureRequest(HttpConnection conn) throws IOException {
			...
			String enc = Base64.getEncoder().encodeToString(ident.getBytes(UTF_8));
	...
	private static class Digest extends org.eclipse.jgit.transport.HttpAuthMethod {
		...
		Digest(String hdr) {
			...
			if ("auth".equals(qop)) { //$NON-NLS-1$
				...
				PRNG.nextBytes(bin);
				params.put("cnonce", java.util.Base64.getEncoder().encodeToString(bin)); //$NON-NLS-1$
			}
		}
	...
	private static class Negotiate extends org.eclipse.jgit.transport.HttpAuthMethod {
		...
		public Negotiate(String hdr) {
			...
			prevToken = java.util.Base64.getDecoder().decode(hdr);
		}
		...
		void configureRequest(HttpConnection conn) throws IOException {
			...
			try {
				...
				conn.setRequestProperty(HDR_AUTHORIZATION, getType().getSchemeName()
						+ " " + Base64.getEncoder().encodeToString(token)); //$NON-NLS-1$
			} catch (GSSException e) {

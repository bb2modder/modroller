...
class WalkFetchConnection extends BaseFetchConnection {
	 ...
	 * objects through.
	 */
	private final List<org.eclipse.jgit.transport.WalkRemoteObjectDatabase> remotes;
	...
	private final LinkedList<org.eclipse.jgit.transport.WalkRemoteObjectDatabase> noPacksYet;

	/** Databases we have not yet obtained the alternates from. */
	private final LinkedList<org.eclipse.jgit.transport.WalkRemoteObjectDatabase> noAlternatesYet;
	...
	WalkFetchConnection(WalkTransport t, org.eclipse.jgit.transport.WalkRemoteObjectDatabase w) {
	...
	public void close() {
		...
		for (org.eclipse.jgit.transport.WalkRemoteObjectDatabase r : remotes)
			r.close();
	}
	...
	private void downloadObject(ProgressMonitor pm, AnyObjectId id)
		...
		for (;;) {
			...
			while (!noPacksYet.isEmpty()) {
				final org.eclipse.jgit.transport.WalkRemoteObjectDatabase wrr = noPacksYet.removeFirst();
			...
			//
			Collection<org.eclipse.jgit.transport.WalkRemoteObjectDatabase> al = expandOneAlternate(id, pm);
			if (al != null && !al.isEmpty()) {
				for (org.eclipse.jgit.transport.WalkRemoteObjectDatabase alt : al) {
					remotes.add(alt);
					noPacksYet.add(alt);
	...
	private boolean downloadLooseObject(final AnyObjectId id,
			final String looseName, final org.eclipse.jgit.transport.WalkRemoteObjectDatabase remote)
			throws TransportException {
		try {
	...
	private Collection<org.eclipse.jgit.transport.WalkRemoteObjectDatabase> expandOneAlternate(
			final AnyObjectId id, final ProgressMonitor pm) {
		while (!noAlternatesYet.isEmpty()) {
			final org.eclipse.jgit.transport.WalkRemoteObjectDatabase wrr = noAlternatesYet.removeFirst();
			try {
				pm.beginTask(JGitText.get().listingAlternates, ProgressMonitor.UNKNOWN);
				Collection<org.eclipse.jgit.transport.WalkRemoteObjectDatabase> altList = wrr
						.getAlternates();
	...
	private class RemotePack {
		...
		RemotePack(org.eclipse.jgit.transport.WalkRemoteObjectDatabase c, String pn) {
			connection = c;
			packName = pn;
		...
		void openIndex(ProgressMonitor pm) throws IOException {
			...
			final org.eclipse.jgit.transport.WalkRemoteObjectDatabase.FileStream s;
		...
		void downloadPack(ProgressMonitor monitor) throws IOException {
			...
			org.eclipse.jgit.transport.WalkRemoteObjectDatabase.FileStream s = connection.open(name);
			try {

...
public class ReceivePack {
		...
		try {
			for (;;) {
				...
				if (firstPkt) {
					...
					if (line.equals(OPTION_PUSH_CERT)) {
	...
	private void receivePack() throws IOException {
		...
		if (sideBand && !quiet)
			resolving = new org.eclipse.jgit.transport.SideBandProgressMonitor(msgOut);
	...
	private void checkConnectivity() throws IOException {
		...
		if (sideBand && !quiet) {
			org.eclipse.jgit.transport.SideBandProgressMonitor m = new org.eclipse.jgit.transport.SideBandProgressMonitor(msgOut);
	...
	protected void executeCommands() {
		...
		if (sideBand) {
			org.eclipse.jgit.transport.SideBandProgressMonitor pm = new org.eclipse.jgit.transport.SideBandProgressMonitor(msgOut);
	...
	private class PostReceiveExecutor implements AutoCloseable {
		@Override
		public void close() {
			postReceive.onPostReceive(org.eclipse.jgit.transport.ReceivePack.this,

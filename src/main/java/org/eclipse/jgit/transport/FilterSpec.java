...
public final class FilterSpec {
	 ...
	 */
	public static org.eclipse.jgit.transport.FilterSpec fromFilterLine(String filterLine)
			throws PackProtocolException {
		if (filterLine.equals("blob:none")) { //$NON-NLS-1$
			...
			if (blobLimit >= 0) {
				return withBlobLimit(blobLimit);
			}
			...
			if (treeDepthLimit >= 0) {
				return withTreeDepthLimit(treeDepthLimit);
			}
		}
	 ...
	 */
	static org.eclipse.jgit.transport.FilterSpec withObjectTypes(ObjectTypes types) {
		return new org.eclipse.jgit.transport.FilterSpec(types, -1, -1);
	}
	 ...
	 */
	static org.eclipse.jgit.transport.FilterSpec withBlobLimit(long blobLimit) {
		if (blobLimit < 0) {
			throw new IllegalArgumentException(
					"blobLimit cannot be negative: " + blobLimit); //$NON-NLS-1$
		}
		return new org.eclipse.jgit.transport.FilterSpec(ObjectTypes.ALL, blobLimit, -1);
	}
	 ...
	 *         depth
	 */
	static org.eclipse.jgit.transport.FilterSpec withTreeDepthLimit(long treeDepthLimit) {
		if (treeDepthLimit < 0) {
			throw new IllegalArgumentException(
					"treeDepthLimit cannot be negative: " + treeDepthLimit); //$NON-NLS-1$
		}
		return new org.eclipse.jgit.transport.FilterSpec(ObjectTypes.ALL, -1, treeDepthLimit);
	}

	/**
	 * A placeholder that indicates no filtering.
	 */
	public static final org.eclipse.jgit.transport.FilterSpec NO_FILTER = new org.eclipse.jgit.transport.FilterSpec(ObjectTypes.ALL, -1, -1);

...
public class UploadPack {
	...
	public interface RequestValidator {
		 ...
		 *
		 * @param up
		 *            {@link org.eclipse.jgit.transport.UploadPack} instance.
		 * @param wants
		 ...
		 * @since 3.1
		 */
		void checkWants(org.eclipse.jgit.transport.UploadPack up, List<ObjectId> wants)
	...
	private void service(PacketLineOut pckOut) throws IOException {
		...
		org.eclipse.jgit.transport.FetchRequest req;
		try {
			if (biDirectionalPipe)
	 ...
	 * client.
	 */
	private void computeShallowsAndUnshallows(org.eclipse.jgit.transport.FetchRequest req,
	...
	private boolean negotiate(org.eclipse.jgit.transport.FetchRequest req,
	...
	public static final class AdvertisedRequestValidator
		...
		@Override
		public void checkWants(org.eclipse.jgit.transport.UploadPack up, List<ObjectId> wants)
				throws PackProtocolException, IOException {
	...
	private static void checkNotAdvertisedWants(org.eclipse.jgit.transport.UploadPack up,
			...
			if (!allWantsAreCommits) {
				...
				try (ObjectWalk objWalk = walk.toObjectWalkWithSameObjects()) {
					Stream<RevObject> startersAsObjs = importantRefsFirst(visibleRefs)
							.map(org.eclipse.jgit.transport.UploadPack::refToObjectId)
	...
	private void sendPack(PackStatistics.Accumulator accumulator,
		...
		if (sideband) {
			...
			if (!req.getClientCapabilities().contains(OPTION_NO_PROGRESS)) {
				...
				pm = new org.eclipse.jgit.transport.SideBandProgressMonitor(msgOut);
			}
	...
	private void sendPack(ProgressMonitor pm, PacketLineOut pckOut,
			OutputStream packOut, org.eclipse.jgit.transport.FetchRequest req,
			PackStatistics.Accumulator accumulator,

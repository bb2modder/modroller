...
public class RemoteConfig implements Serializable {
	 ...
	 */
	public static List<org.eclipse.jgit.transport.RemoteConfig> getAllRemoteConfigs(Config rc)
			throws URISyntaxException {
		final List<String> names = new ArrayList<>(rc
				.getSubsections(SECTION));
		Collections.sort(names);

		final List<org.eclipse.jgit.transport.RemoteConfig> result = new ArrayList<>(names
				.size());
		for (String name : names)
			result.add(new org.eclipse.jgit.transport.RemoteConfig(rc, name));
		return result;
	}

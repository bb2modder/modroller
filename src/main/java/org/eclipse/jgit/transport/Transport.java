...
public abstract class Transport implements AutoCloseable {
	...
	static {
		...
		register(org.eclipse.jgit.transport.TransportGitAnon.PROTO_GIT);
	...
	private static void registerByService() {
		...
		if (ldr == null)
			ldr = org.eclipse.jgit.transport.Transport.class.getClassLoader();
	...
	private static Enumeration<URL> catalogs(ClassLoader ldr) {
		try {
			...
			String name = prefix + org.eclipse.jgit.transport.Transport.class.getName();
	 ...
	 * {@link TransportProtocol}'s class
	 ...
	 */
	public static org.eclipse.jgit.transport.Transport open(Repository local, String remote)
	 ...
	 *             the transport cannot open this URI.
	 */
	public static org.eclipse.jgit.transport.Transport open(final Repository local, final String remote,
	 ...
	 *             the transport cannot open this URI.
	 */
	public static List<org.eclipse.jgit.transport.Transport> openAll(final Repository local,
	...
	public static List<org.eclipse.jgit.transport.Transport> openAll(final Repository local,
		...
		final RemoteConfig cfg = new RemoteConfig(local.getConfig(), remote);
		if (doesNotExist(cfg)) {
			final ArrayList<org.eclipse.jgit.transport.Transport> transports = new ArrayList<>(1);
	 ...
	 *             associated.
	 */
	public static org.eclipse.jgit.transport.Transport open(Repository local, RemoteConfig cfg)
	 ...
	 *             associated.
	 */
	public static org.eclipse.jgit.transport.Transport open(final Repository local,
		...
		final org.eclipse.jgit.transport.Transport tn = open(local, uris.get(0), cfg.getName());
		tn.applyConfig(cfg);
		return tn;
	}
	...
	public static List<org.eclipse.jgit.transport.Transport> openAll(final Repository local,
			...
			throws NotSupportedException, TransportException {
		final List<URIish> uris = getURIs(cfg, op);
		final List<org.eclipse.jgit.transport.Transport> transports = new ArrayList<>(uris.size());
		for (URIish uri : uris) {
			final org.eclipse.jgit.transport.Transport tn = open(local, uri, cfg.getName());
			tn.applyConfig(cfg);
			transports.add(tn);
		}
		return transports;
	 ...
	 *             the transport cannot open this URI.
	 */
	public static org.eclipse.jgit.transport.Transport open(Repository local, URIish uri)
			throws NotSupportedException, TransportException {
	 ...
	 *             the transport cannot open this URI.
	 */
	public static org.eclipse.jgit.transport.Transport open(Repository local, URIish uri, String remoteName)
			throws NotSupportedException, TransportException {
		for (WeakReference<TransportProtocol> ref : protocols) {
			TransportProtocol proto = ref.get();
			if (proto == null) {
			...
			if (proto.canHandle(uri, local, remoteName)) {
				org.eclipse.jgit.transport.Transport tn = proto.open(uri, local, remoteName);
	 ...
	 * @throws TransportException
	 */
	public static org.eclipse.jgit.transport.Transport open(URIish uri) throws NotSupportedException, TransportException {
		for (WeakReference<TransportProtocol> ref : protocols) {
			TransportProtocol proto = ref.get();
			if (proto == null) {
				protocols.remove(ref);
	 ...
	 * {@link RefSpec} form to
	 * {@link RemoteRefUpdate}. Conversion expands
	...
	/**
	 * Default setting is:
	 * {@link RemoteConfig#DEFAULT_RECEIVE_PACK}
	 *
	 ...
	 * {@link RemoteConfig} with fetch RefSpecs
	 * defined.
	 *
	 * @param monitor
	 ...
	 * directly {@link RemoteRefUpdate} for more
	 * possibilities.
	 * <p>

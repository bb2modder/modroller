...
public class TransportHttp extends HttpTransport implements WalkTransport,
	...
	static final TransportProtocol PROTO_HTTP = new TransportProtocol() {
		...
		public Transport open(URIish uri, Repository local, String remoteName)
			...
			return new org.eclipse.jgit.transport.TransportHttp(local, uri);
		}

		@Override
		public Transport open(URIish uri) throws NotSupportedException {
			return new org.eclipse.jgit.transport.TransportHttp(uri);
		}
	};
	...
	private org.eclipse.jgit.transport.HttpAuthMethod authMethod = Type.NONE.method(null);
	...
	private FetchConnection getConnection(HttpConnection c, InputStream in,
		...
		f.setPeerUserAgent(c.getHeaderField(HDR_SERVER));
	...
	private PushConnection smartPush(String service, HttpConnection c,
		...
		p.setPeerUserAgent(c.getHeaderField(HDR_SERVER));
		return p;
	}
	...
	private HttpConnection connect(String service)
		...
		for (;;) {
			try {
				...
				final int status = response(conn);
				...
				case HttpConnection.HTTP_OK:
					...
					if (authMethod.getType() == Type.NONE
						...
						authMethod = org.eclipse.jgit.transport.HttpAuthMethod.scanResponse(conn, ignoreTypes);
					return conn;
	...
	protected HttpConnection httpOpen(String method, URL u,
		...
		final Proxy proxy = proxyFor(proxySelector, u);
	...
	class HttpObjectDB extends org.eclipse.jgit.transport.WalkRemoteObjectDatabase {
		...
		@Override
		Collection<org.eclipse.jgit.transport.WalkRemoteObjectDatabase> getAlternates() throws IOException {
			try {
		...
		@Override
		org.eclipse.jgit.transport.WalkRemoteObjectDatabase openAlternate(String location)
				throws IOException {
	...
	class SmartHttpFetchConnection extends BasePackFetchConnection {
		...
		SmartHttpFetchConnection(InputStream advertisement)
				throws TransportException {
			super(org.eclipse.jgit.transport.TransportHttp.this);
			statelessRPC = true;
	...
	abstract class Service {
		...
		void sendRequest() throws IOException {
			...
			org.eclipse.jgit.transport.HttpAuthMethod authenticator = null;
			...
			for (;;) {
				try {
					...
					case HttpConnection.HTTP_UNAUTHORIZED:
						org.eclipse.jgit.transport.HttpAuthMethod nextMethod = org.eclipse.jgit.transport.HttpAuthMethod
						...
						case NEGOTIATE:
							...
							ignoreTypes.add(Type.NEGOTIATE);

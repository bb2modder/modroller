...
public class AddCommand extends GitCommand<DirCache> {
	 ...
	 * @return {@code this}
	 */
	public org.eclipse.jgit.api.AddCommand addFilepattern(String filepattern) {
		checkCallable();
	 ...
	 * @return {@code this}
	 */
	public org.eclipse.jgit.api.AddCommand setWorkingTreeIterator(WorkingTreeIterator f) {
		...
		try (ObjectInserter inserter = repo.newObjectInserter();
			...
			while (tw.next()) {
				...
				if ((f.getEntryRawMode() == TYPE_TREE
						&& f.getIndexFileMode(c) != GITLINK) ||
						(f.getEntryRawMode() == TYPE_GITLINK
								&& f.getIndexFileMode(c) == TREE)) {
	 ...
	 * @return {@code this}
	 */
	public org.eclipse.jgit.api.AddCommand setUpdate(boolean update) {
		this.update = update;
		return this;
	}

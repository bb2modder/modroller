...
public class BlockList<T> extends AbstractList<T> {
	 ...
	 */
	public BlockList() {
		directory = org.eclipse.jgit.util.BlockList.<T> newDirectory(256);
		directory[0] = org.eclipse.jgit.util.BlockList.<T> newBlock();
	...
	public BlockList(int capacity) {
		...
		if ((capacity & BLOCK_MASK) != 0 || dirSize == 0)
			dirSize++;
		directory = org.eclipse.jgit.util.BlockList.<T> newDirectory(dirSize);
		directory[0] = org.eclipse.jgit.util.BlockList.<T> newBlock();
		tailBlock = directory[0];
	}
	 ...
	 */
	public void addAll(org.eclipse.jgit.util.BlockList<T> src) {
		if (src.size == 0)
			return;
	...
	public boolean add(T element) {
		...
		if (++tailDirIdx == directory.length) {
			T[][] newDir = org.eclipse.jgit.util.BlockList.<T> newDirectory(directory.length << 1);
		...
		if (blockRef == null) {
			blockRef = org.eclipse.jgit.util.BlockList.<T> newBlock();
	...
	private class MyIterator implements Iterator<T> {
		...
		public void remove() {
			...
			org.eclipse.jgit.util.BlockList.this.remove(--index);

...
public class RefList<T extends Ref> implements Iterable<Ref> {
	private static final org.eclipse.jgit.util.RefList<Ref> EMPTY = new org.eclipse.jgit.util.RefList<>(new Ref[0], 0);
	...
	public static <T extends Ref> org.eclipse.jgit.util.RefList<T> emptyList() {
		return (org.eclipse.jgit.util.RefList<T>) EMPTY;
	}
	 ...
	 *            the source list.
	 */
	protected RefList(org.eclipse.jgit.util.RefList<T> src) {
		this.list = src.list;
		this.cnt = src.cnt;
	}
	 ...
	 */
	public final org.eclipse.jgit.util.RefList<T> set(int idx, T ref) {
		Ref[] newList = new Ref[cnt];
		System.arraycopy(list, 0, newList, 0, cnt);
		newList[idx] = ref;
		return new org.eclipse.jgit.util.RefList<>(newList, cnt);
	}
	 ...
	 */
	public final org.eclipse.jgit.util.RefList<T> add(int idx, T ref) {
		if (idx < 0)
			idx = -(idx + 1);
		...
		return new org.eclipse.jgit.util.RefList<>(newList, cnt + 1);
	}
	 ...
	 */
	public final org.eclipse.jgit.util.RefList<T> remove(int idx) {
		if (cnt == 1)
			return emptyList();
		Ref[] newList = new Ref[cnt - 1];
		if (0 < idx)
	 ...
	 */
	public final org.eclipse.jgit.util.RefList<T> put(T ref) {
	 ...
	 * @since 5.4
	 */
	public static <T extends Ref> Collector<T, ?, org.eclipse.jgit.util.RefList<T>> toRefList(
	...
	public static class Builder<T extends Ref> {
		...
		public org.eclipse.jgit.util.RefList<T> toRefList() {
			return new org.eclipse.jgit.util.RefList<>(list, size);
		}

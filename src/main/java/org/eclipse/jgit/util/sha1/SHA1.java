...
public class SHA1 {
	...
	public static org.eclipse.jgit.util.sha1.SHA1 newInstance() {
		return new org.eclipse.jgit.util.sha1.SHA1();
	}
	 ...
	 * @return {@code this}
	 */
	public org.eclipse.jgit.util.sha1.SHA1 setDetectCollision(boolean detect) {
	...
	private void compress(byte[] block, int p) {
		initBlock(block, p);
		int ubcDvMask = detectCollision ? org.eclipse.jgit.util.sha1.UbcCheck.check(w) : 0;
		compress();

		while (ubcDvMask != 0) {
			int b = numberOfTrailingZeros(lowestOneBit(ubcDvMask));
			org.eclipse.jgit.util.sha1.UbcCheck.DvInfo dv = org.eclipse.jgit.util.sha1.UbcCheck.DV[b];
			for (int i = 0; i < 80; i++) {
				w2[i] = w[i] ^ dv.dm[i];
			}
			recompress(dv.testt);

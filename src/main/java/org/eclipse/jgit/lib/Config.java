...
public class Config {
	 ...
	 */
	public Config(org.eclipse.jgit.lib.Config defaultConfig) {
	...
	private ConfigSnapshot unsetSection(final ConfigSnapshot srcState,
		...
		final ArrayList<org.eclipse.jgit.lib.ConfigLine> r = new ArrayList<>(max);

		boolean lastWasMatch = false;
		for (org.eclipse.jgit.lib.ConfigLine e : srcState.entryList) {
	...
	private ConfigSnapshot replaceStringList(final ConfigSnapshot srcState,
		...
		final List<org.eclipse.jgit.lib.ConfigLine> entries = copy(srcState, values);
		int entryIndex = 0;
		int valueIndex = 0;
		...
		while (entryIndex < entries.size() && valueIndex < values.size()) {
			final org.eclipse.jgit.lib.ConfigLine e = entries.get(entryIndex);
		...
		if (valueIndex < values.size() && entryIndex == entries.size()) {
			...
			if (insertPosition < 0) {
				...
				//
				final org.eclipse.jgit.lib.ConfigLine e = new org.eclipse.jgit.lib.ConfigLine();
				e.section = section;
	...
	private static List<org.eclipse.jgit.lib.ConfigLine> copy(final ConfigSnapshot src,
	                                                          final List<String> values) {
	...
	private static int findSectionEnd(final List<org.eclipse.jgit.lib.ConfigLine> entries,
			final String section, final String subsection,
			boolean skipIncludedLines) {
		for (int i = 0; i < entries.size(); i++) {
			org.eclipse.jgit.lib.ConfigLine e = entries.get(i);
	...
	public String toText() {
		...
		for (org.eclipse.jgit.lib.ConfigLine e : state.get().entryList) {
			if (e.includedFrom != null)
				continue;
			if (e.prefix != null)
				out.append(e.prefix);
	...
	private List<org.eclipse.jgit.lib.ConfigLine> fromTextRecurse(String text, int depth,
	                                                              String includedFrom) throws ConfigInvalidException {
		if (depth > MAX_DEPTH) {
			throw new ConfigInvalidException(
					JGitText.get().tooManyIncludeRecursions);
		}
		final List<org.eclipse.jgit.lib.ConfigLine> newEntries = new ArrayList<>();
		final StringReader in = new StringReader(text);
		org.eclipse.jgit.lib.ConfigLine last = null;
		org.eclipse.jgit.lib.ConfigLine e = new org.eclipse.jgit.lib.ConfigLine();
		...
		for (;;) {
			...
			if ('\n' == c) {
				...
				if (e.section != null)
					last = e;
				e = new org.eclipse.jgit.lib.ConfigLine();
				e.includedFrom = includedFrom;
	...
	private void addIncludedConfig(final List<org.eclipse.jgit.lib.ConfigLine> newEntries,
	                               org.eclipse.jgit.lib.ConfigLine line, int depth) throws ConfigInvalidException {
	...
	private ConfigSnapshot newState() {
		return new ConfigSnapshot(Collections.<org.eclipse.jgit.lib.ConfigLine> emptyList(),
				getBaseState());
	}

	private ConfigSnapshot newState(List<org.eclipse.jgit.lib.ConfigLine> entries) {
	...
	public static interface SectionParser<T> {
		 ...
		 */
		T parse(org.eclipse.jgit.lib.Config cfg);
	}

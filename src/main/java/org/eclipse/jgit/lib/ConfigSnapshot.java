...
class ConfigSnapshot {
	final List<org.eclipse.jgit.lib.ConfigLine> entryList;
	final Map<Object, Object> cache;
	final org.eclipse.jgit.lib.ConfigSnapshot baseState;
	volatile List<org.eclipse.jgit.lib.ConfigLine> sorted;
	volatile SectionNames names;

	ConfigSnapshot(List<org.eclipse.jgit.lib.ConfigLine> entries, org.eclipse.jgit.lib.ConfigSnapshot base) {
		entryList = entries;
	...
	private Map<String, String> getNamesInternal(String section,
		...
		List<org.eclipse.jgit.lib.ConfigLine> s = sorted();
		...
		while (idx < s.size()) {
			org.eclipse.jgit.lib.ConfigLine e = s.get(idx++);
	...
	private int find(List<org.eclipse.jgit.lib.ConfigLine> s, String s1, String s2, String name) {
		int low = 0;
		int high = s.size();
		while (low < high) {
			int mid = (low + high) >>> 1;
			org.eclipse.jgit.lib.ConfigLine e = s.get(mid);
			int cmp = compare2(
					s1, s2, name,
	...
	private int first(List<org.eclipse.jgit.lib.ConfigLine> s, int i, String s1, String s2, String n) {
		while (0 < i) {
	...
	private int end(List<org.eclipse.jgit.lib.ConfigLine> s, int i, String s1, String s2, String n) {
		while (i < s.size()) {
	...
	private List<org.eclipse.jgit.lib.ConfigLine> sorted() {
		List<org.eclipse.jgit.lib.ConfigLine> r = sorted;
		if (r == null)
			sorted = r = sort(entryList);
		return r;
	}

	private static List<org.eclipse.jgit.lib.ConfigLine> sort(List<org.eclipse.jgit.lib.ConfigLine> in) {
		List<org.eclipse.jgit.lib.ConfigLine> sorted = new ArrayList<>(in.size());
		for (org.eclipse.jgit.lib.ConfigLine line : in) {
	...
	private static class LineComparator implements Comparator<org.eclipse.jgit.lib.ConfigLine> {
		@Override
		public int compare(org.eclipse.jgit.lib.ConfigLine a, org.eclipse.jgit.lib.ConfigLine b) {
			return compare2(
	...
	private static class SectionNames {
		...
		SectionNames(org.eclipse.jgit.lib.ConfigSnapshot cfg) {
			Map<String, String> sec = new LinkedHashMap<>();
			Map<String, Set<String>> sub = new HashMap<>();
			while (cfg != null) {
				for (org.eclipse.jgit.lib.ConfigLine e : cfg.entryList) {
					if (e.section == null)
						continue;
		...
		public boolean contains(Object needle) {
			if (needle instanceof String) {
				...
				return names.containsKey(n)
						|| names.containsKey(toLowerCase(n));
			}
			return false;
		}

 ...
 */
class FanoutBucket extends org.eclipse.jgit.notes.InMemoryNoteBucket {
	/**
	 ...
	 */
	private final org.eclipse.jgit.notes.NoteBucket[] table;
	...
	FanoutBucket(int prefixLen) {
		super(prefixLen);
		table = new org.eclipse.jgit.notes.NoteBucket[256];
	}
	...
	void setBucket(int cell, org.eclipse.jgit.notes.InMemoryNoteBucket bucket) {
		table[cell] = bucket;
		cnt++;
	}
	...
	Note getNote(AnyObjectId objId, ObjectReader or) throws IOException {
		org.eclipse.jgit.notes.NoteBucket b = table[cell(objId)];
		return b != null ? b.getNote(objId, or) : null;

	}

	org.eclipse.jgit.notes.NoteBucket getBucket(int cell) {
		return table[cell];
	}

	static org.eclipse.jgit.notes.InMemoryNoteBucket loadIfLazy(org.eclipse.jgit.notes.NoteBucket b, AnyObjectId prefix,
	                                                            ObjectReader or) throws IOException {
		if (b == null)
			return null;
		if (b instanceof org.eclipse.jgit.notes.InMemoryNoteBucket)
			return (org.eclipse.jgit.notes.InMemoryNoteBucket) b;
		...
		return new Iterator<Note>() {
			...
			public boolean hasNext() {
				...
				for (; cell < table.length; cell++) {
					org.eclipse.jgit.notes.NoteBucket b = table[cell];
					if (b == null)
						continue;
	...
	int estimateSize(AnyObjectId noteOn, ObjectReader or) throws IOException {
		...
		if (org.eclipse.jgit.notes.LeafBucket.MAX_SIZE * 3 / 4 <= cnt)
			return 1 + org.eclipse.jgit.notes.LeafBucket.MAX_SIZE;
		...
		for (int cell = 0; cell < 256; cell++) {
			...
			if (org.eclipse.jgit.notes.LeafBucket.MAX_SIZE < sz)
				break;
		}
		return sz;
	}
	...
	@Override
	org.eclipse.jgit.notes.InMemoryNoteBucket set(AnyObjectId noteOn, AnyObjectId noteData,
	                                              ObjectReader or) throws IOException {
		...
		if (b == null) {
			...
			org.eclipse.jgit.notes.LeafBucket n = new org.eclipse.jgit.notes.LeafBucket(prefixLen + 2);
			table[cell] = n.set(noteOn, noteData, or);
			cnt++;
			return this;

		}
		org.eclipse.jgit.notes.NoteBucket n = b.set(noteOn, noteData, or);
		if (n == null) {
			table[cell] = null;
			cnt--;
	...
	org.eclipse.jgit.notes.InMemoryNoteBucket contractIfTooSmall(AnyObjectId noteOn, ObjectReader or)
			throws IOException {
		if (estimateSize(noteOn, or) < org.eclipse.jgit.notes.LeafBucket.MAX_SIZE) {
			// We are small enough to just contract to a single leaf.
			org.eclipse.jgit.notes.InMemoryNoteBucket r = new org.eclipse.jgit.notes.LeafBucket(prefixLen);
	...
	private TreeFormatter build(boolean insert, ObjectInserter inserter)
		...
		org.eclipse.jgit.notes.NonNoteEntry e = nonNotes;
	...
	private int treeSize() {
		...
		for (org.eclipse.jgit.notes.NonNoteEntry e = nonNotes; e != null; e = e.next)
			sz += e.treeEntrySize();
		return sz;
	}

	@Override
	org.eclipse.jgit.notes.InMemoryNoteBucket append(Note note) {
		int cell = cell(note);
		org.eclipse.jgit.notes.InMemoryNoteBucket b = (org.eclipse.jgit.notes.InMemoryNoteBucket) table[cell];
		...
		if (b == null) {
			...
			org.eclipse.jgit.notes.InMemoryNoteBucket n = b.append(note);
			if (n != b)
				table[cell] = n;
		}
		return this;
	...
	private class LazyNoteBucket extends org.eclipse.jgit.notes.NoteBucket {
		...
		private org.eclipse.jgit.notes.InMemoryNoteBucket load(AnyObjectId prefix, ObjectReader or)
				throws IOException {
			AbbreviatedObjectId p = prefix.abbreviate(prefixLen + 2);
			org.eclipse.jgit.notes.InMemoryNoteBucket self = org.eclipse.jgit.notes.NoteParser.parse(p, treeId, or);

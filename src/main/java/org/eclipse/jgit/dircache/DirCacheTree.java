...
public class DirCacheTree {
	...
	private static final Comparator<org.eclipse.jgit.dircache.DirCacheTree> TREE_CMP = (org.eclipse.jgit.dircache.DirCacheTree o1,
	                                                                                    org.eclipse.jgit.dircache.DirCacheTree o2) -> {
	...
	private DirCacheTree(final org.eclipse.jgit.dircache.DirCacheTree myParent, final byte[] path,
	...
	DirCacheTree(final byte[] in, final MutableInteger off,
			final org.eclipse.jgit.dircache.DirCacheTree myParent) {
		parent = myParent;
		...
		if (subcnt > 0) {
			...
			children = new org.eclipse.jgit.dircache.DirCacheTree[subcnt];
			for (int i = 0; i < subcnt; i++) {
				children[i] = new org.eclipse.jgit.dircache.DirCacheTree(in, off, this);
	 ...
	 */
	public org.eclipse.jgit.dircache.DirCacheTree getChild(int i) {
		return children[i];
	}
		...
		if (id == null) {
			...
			while (entryIdx < endIdx) {
				...
				if (childIdx < childCnt) {
					final org.eclipse.jgit.dircache.DirCacheTree st = children[childIdx];
	...
	void validate(final DirCacheEntry[] cache, final int cCnt, int cIdx,
		...
		while (cIdx < cCnt) {
			...
			org.eclipse.jgit.dircache.DirCacheTree st = stIdx < childCnt ? children[stIdx] : null;
			...
			if (cc < 0) {
				...
				//
				st = new org.eclipse.jgit.dircache.DirCacheTree(this, currPath, pathOff, p - pathOff);
	...
	private void insertChild(int stIdx, org.eclipse.jgit.dircache.DirCacheTree st) {
		final org.eclipse.jgit.dircache.DirCacheTree[] c = children;
		...
		final org.eclipse.jgit.dircache.DirCacheTree[] a = new org.eclipse.jgit.dircache.DirCacheTree[n + 1];
		if (stIdx > 0)
	...
	private static int namecmp(byte[] a, int aPos, org.eclipse.jgit.dircache.DirCacheTree ct) {
		if (ct == null)
			return -1;

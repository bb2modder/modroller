...
public class PlotCommit<L extends PlotLane> extends RevCommit {
	static final org.eclipse.jgit.revplot.PlotCommit[] NO_CHILDREN = {};
	...
	org.eclipse.jgit.revplot.PlotCommit[] children;
	...
	void addChild(org.eclipse.jgit.revplot.PlotCommit c) {
		final int cnt = children.length;
		switch (cnt) {
		case 0:
			children = new org.eclipse.jgit.revplot.PlotCommit[] { c };
			break;
		case 1:
			if (!c.getId().equals(children[0].getId()))
				children = new org.eclipse.jgit.revplot.PlotCommit[] { children[0], c };
			break;
		default:
			for (org.eclipse.jgit.revplot.PlotCommit pc : children)
				if (c.getId().equals(pc.getId()))
					return;
			final org.eclipse.jgit.revplot.PlotCommit[] n = new org.eclipse.jgit.revplot.PlotCommit[cnt + 1];
	 ...
	 */
	public final org.eclipse.jgit.revplot.PlotCommit getChild(int nth) {
		return children[nth];
	}
	 ...
	 */
	public final boolean isChild(org.eclipse.jgit.revplot.PlotCommit c) {
		for (org.eclipse.jgit.revplot.PlotCommit a : children)
			if (a == c)
				return true;
		return false;
	}

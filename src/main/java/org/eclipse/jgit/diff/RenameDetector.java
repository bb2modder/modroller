...
public class RenameDetector {
	...
	private void findContentRenames(ContentSource.Pair reader,
		...
		if (getRenameLimit() == 0 || cnt <= getRenameLimit()) {
			org.eclipse.jgit.diff.SimilarityRenameDetector d;

			d = new org.eclipse.jgit.diff.SimilarityRenameDetector(reader, deleted, added);
			...
			if (o instanceof DiffEntry) {
				...
				for (int delIdx = 0; delIdx < dels.size(); delIdx++) {
					...
					for (int addIdx = 0; addIdx < adds.size(); addIdx++) {
						...
						int score = org.eclipse.jgit.diff.SimilarityRenameDetector.nameScore(addedName, deletedName);
						matrix[mNext] = org.eclipse.jgit.diff.SimilarityRenameDetector.encode(score, delIdx, addIdx);
						mNext++;
						if (pm.isCancelled()) {
				...
				for (--mNext; mNext >= 0; mNext--) {
					...
					int delIdx = org.eclipse.jgit.diff.SimilarityRenameDetector.srcFile(ent);
					int addIdx = org.eclipse.jgit.diff.SimilarityRenameDetector.dstFile(ent);
	...
	private static DiffEntry bestPathMatch(DiffEntry src, List<DiffEntry> list) {
		...
		for (DiffEntry d : list) {
			if (sameType(mode(d), mode(src))) {
				int tmp = org.eclipse.jgit.diff.SimilarityRenameDetector

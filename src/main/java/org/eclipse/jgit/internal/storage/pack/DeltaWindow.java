...
final class DeltaWindow {
	...
	private final org.eclipse.jgit.internal.storage.pack.DeltaCache deltaCache;
	...
	private org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry res;

	/** If we have chosen a base, the window entry it was created from. */
	private org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry bestBase;
	...
	DeltaWindow(PackConfig pc, org.eclipse.jgit.internal.storage.pack.DeltaCache dc, ObjectReader or,
		...
		res = org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry.createWindow(config.getDeltaSearchWindowSize());
	}
	...
	synchronized org.eclipse.jgit.internal.storage.pack.DeltaTask.Slice remaining() {
		...
		for (int n = split + 1; n < e; n++) {
			if (h != toSearch[n].getPathHash())
				return new org.eclipse.jgit.internal.storage.pack.DeltaTask.Slice(n, e);
		}
		...
		if (h != toSearch[cur].getPathHash()) {
			...
			for (int p = split - 1; cur < p; p--) {
				if (h != toSearch[p].getPathHash())
					return new org.eclipse.jgit.internal.storage.pack.DeltaTask.Slice(p + 1, e);
			}
		}
		return null;
	}

	synchronized boolean tryStealWork(org.eclipse.jgit.internal.storage.pack.DeltaTask.Slice s) {
		...
		try {
			for (;;) {
				...
				if (maxMemory != 0) {
					...
					org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry n = res.next;
	...
	private static long estimateIndexSize(org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry ent) {
		if (ent.buffer == null)
	...
	private void clearWindowOnTypeSwitch() {
		org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry p = res.prev;
	...
	private void clear(org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry ent) {
		if (ent.index != null)
	...
	private void searchInWindow() throws IOException {
		...
		for (org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry src = res.prev; src != res; src = src.prev) {
			if (src.empty())
				break;
	...
	private boolean delta(org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry src)
			throws IOException {
	...
	private void selectDeltaBase(org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry src, OutputStream delta) {
		bestBase = src;
	...
	private int deltaSizeLimit(org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry src) {
		if (bestBase == null) {
	...
	private DeltaIndex index(org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry ent)
	...
	private byte[] buffer(org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry ent) throws MissingObjectException,
	...
	private void checkLoadable(org.eclipse.jgit.internal.storage.pack.DeltaWindowEntry ent, long need) {
		if (maxMemory == 0)
			return;

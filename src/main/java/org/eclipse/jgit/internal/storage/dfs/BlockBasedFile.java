...
abstract class BlockBasedFile {
	...
	org.eclipse.jgit.internal.storage.dfs.DfsBlock getOrLoadBlock(long pos, DfsReader ctx) throws IOException {
		try (LazyChannel c = new LazyChannel(ctx, desc, ext)) {
			return cache.getOrLoad(this, pos, ctx, c);
		}
	}

	org.eclipse.jgit.internal.storage.dfs.DfsBlock readOneBlock(long pos, DfsReader ctx, ReadableChannel rc)
			throws IOException {
		if (invalid) {
		...
		try {
			...
			return new org.eclipse.jgit.internal.storage.dfs.DfsBlock(key, pos, buf);
		} finally {

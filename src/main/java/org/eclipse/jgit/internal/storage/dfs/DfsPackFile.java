...
public final class DfsPackFile extends BlockBasedFile {
	...
	private void copyPackThroughCache(PackOutputStream out, DfsReader ctx,
		...
		while (0 < remaining) {
			org.eclipse.jgit.internal.storage.dfs.DfsBlock b = cache.getOrLoad(this, position, ctx, () -> rc);
	...
	private long copyPackBypassCache(PackOutputStream out, ReadableChannel rc)
		...
		while (0 < remaining) {
			org.eclipse.jgit.internal.storage.dfs.DfsBlock b = cache.get(key, alignToBlock(position));
			if (b != null) {
				int ptr = (int) (position - b.start);
				if (b.size() <= ptr) {
					throw packfileIsTruncated();
	...
	void copyAsIs(PackOutputStream out, DfsObjectToPack src,
		...
		final org.eclipse.jgit.internal.storage.dfs.DfsBlock quickCopy;
		...
		try {
			...
			SEARCH: for (;;) {
				...
				case Constants.OBJ_TAG: {
					...
					return new org.eclipse.jgit.internal.storage.dfs.LargePackedWholeObject(typeCode, sz, pos, p, this, ctx.db);
				}
				...
				case Constants.OBJ_OFS_DELTA: {
					...
					org.eclipse.jgit.internal.storage.dfs.DeltaBaseCache.Entry e = ctx.getDeltaBaseCache().get(key, base);
					if (e != null) {
						type = e.type;
						data = e.data;
						cached = true;

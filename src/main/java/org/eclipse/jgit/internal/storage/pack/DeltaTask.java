...
final class DeltaTask implements Callable<Object> {
	...
	static final class Block {
		...
		final List<org.eclipse.jgit.internal.storage.pack.DeltaTask> tasks;
		final int threads;
		final PackConfig config;
		final ObjectReader templateReader;
		final org.eclipse.jgit.internal.storage.pack.DeltaCache dc;
		...
		Block(int threads, PackConfig config, ObjectReader reader,
		      org.eclipse.jgit.internal.storage.pack.DeltaCache dc, ThreadSafeProgressMonitor pm,
		...
		synchronized DeltaWindow stealWork(org.eclipse.jgit.internal.storage.pack.DeltaTask forThread) {
			for (;;) {
				org.eclipse.jgit.internal.storage.pack.DeltaTask maxTask = null;
				Slice maxSlice = null;
				int maxWork = 0;

				for (org.eclipse.jgit.internal.storage.pack.DeltaTask task : tasks) {
		...
		void partitionTasks() {
			...
			for (int i = beginIndex; i < endIndex;) {
				org.eclipse.jgit.internal.storage.pack.DeltaTask task = new org.eclipse.jgit.internal.storage.pack.DeltaTask(this);
				long w = 0;

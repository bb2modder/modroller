...
public class DfsInserter extends ObjectInserter {
	...
	private class PackStream extends OutputStream {
		...
		private void flushBlock() throws IOException {
			...
			cache.put(new org.eclipse.jgit.internal.storage.dfs.DfsBlock(packKey, currPos, buf));
		...
		int read(long pos, byte[] dst, int ptr, int cnt) throws IOException {
			...
			while (pos < currPos && r < cnt) {
				org.eclipse.jgit.internal.storage.dfs.DfsBlock b = getOrLoadBlock(pos);
		...
		private org.eclipse.jgit.internal.storage.dfs.DfsBlock getOrLoadBlock(long pos) throws IOException {
			long s = toBlockStart(pos);
			org.eclipse.jgit.internal.storage.dfs.DfsBlock b = cache.get(packKey, s);
			if (b != null)
				return b;
			...
			b = new org.eclipse.jgit.internal.storage.dfs.DfsBlock(packKey, s, d);
			cache.put(b);
			return b;
		}
	...
	private class Reader extends ObjectReader {
		...
		public ObjectInserter getCreatedFromInserter() {
			return org.eclipse.jgit.internal.storage.dfs.DfsInserter.this;
		}

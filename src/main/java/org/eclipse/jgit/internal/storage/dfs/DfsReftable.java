...
public class DfsReftable extends BlockBasedFile {
	...
	private static final class CacheSource extends BlockSource {
		private final org.eclipse.jgit.internal.storage.dfs.DfsReftable file;
		private final DfsBlockCache cache;
		private final DfsReader ctx;
		private ReadableChannel ch;
		private int readAhead;

		CacheSource(org.eclipse.jgit.internal.storage.dfs.DfsReftable file, DfsBlockCache cache, DfsReader ctx) {
			this.file = file;
			this.cache = cache;
			this.ctx = ctx;
		}
		...
		public ByteBuffer read(long pos, int cnt) throws IOException {
			...
			org.eclipse.jgit.internal.storage.dfs.DfsBlock block = cache.getOrLoad(file, pos, ctx, () -> open());

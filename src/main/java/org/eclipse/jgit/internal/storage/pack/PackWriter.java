...
public class PackWriter implements AutoCloseable {
	...
	private static final Map<WeakReference<org.eclipse.jgit.internal.storage.pack.PackWriter>, Boolean> instances =
			new ConcurrentHashMap<>();

	private static final Iterable<org.eclipse.jgit.internal.storage.pack.PackWriter> instancesIterable = () -> new Iterator<org.eclipse.jgit.internal.storage.pack.PackWriter>() {

		private final Iterator<WeakReference<org.eclipse.jgit.internal.storage.pack.PackWriter>> it = instances
				.keySet().iterator();

		private org.eclipse.jgit.internal.storage.pack.PackWriter next;
		...
		public boolean hasNext() {
			...
			while (it.hasNext()) {
				WeakReference<org.eclipse.jgit.internal.storage.pack.PackWriter> ref = it.next();
				next = ref.get();
				if (next != null) {
					return true;
				}
		...
		@Override
		public org.eclipse.jgit.internal.storage.pack.PackWriter next() {
			if (hasNext()) {
				org.eclipse.jgit.internal.storage.pack.PackWriter result = next;
				next = null;
				return result;
			}
	 ...
	 */
	public static Iterable<org.eclipse.jgit.internal.storage.pack.PackWriter> getInstances() {
	...
	private final WeakReference<org.eclipse.jgit.internal.storage.pack.PackWriter> selfRef;
	 ...
	 */
	public org.eclipse.jgit.internal.storage.pack.PackWriter setObjectCountCallback(ObjectCountCallback callback) {
	...
	private void singleThreadDeltaSearch(ProgressMonitor monitor,
		...
		for (int i = 0; i < cnt; i++) {
			...
			totalWeight += org.eclipse.jgit.internal.storage.pack.DeltaTask.getAdjustedWeight(o);
		}

		long bytesPerUnit = 1;
		while (org.eclipse.jgit.internal.storage.pack.DeltaTask.MAX_METER <= (totalWeight / bytesPerUnit))
			bytesPerUnit <<= 10;
		...
		new DeltaWindow(config, new org.eclipse.jgit.internal.storage.pack.DeltaCache(config), reader,
	...
	private void parallelDeltaSearch(ProgressMonitor monitor,
		...
		org.eclipse.jgit.internal.storage.pack.DeltaCache dc = new org.eclipse.jgit.internal.storage.pack.ThreadSafeDeltaCache(config);
		ThreadSafeProgressMonitor pm = new ThreadSafeProgressMonitor(monitor);
		org.eclipse.jgit.internal.storage.pack.DeltaTask.Block taskBlock = new org.eclipse.jgit.internal.storage.pack.DeltaTask.Block(threads, config,
				reader, dc, pm,
				list, 0, cnt);
		...
		if (executor instanceof ExecutorService) {
			...
			for (org.eclipse.jgit.internal.storage.pack.DeltaTask task : taskBlock.tasks) {
				executor.execute(() -> {
					try {
						task.call();
	...
	private static void runTasks(ExecutorService pool,
	                             ...
	                             org.eclipse.jgit.internal.storage.pack.DeltaTask.Block tb, List<Throwable> errors) throws IOException {
		List<Future<?>> futures = new ArrayList<>(tb.tasks.size());
		for (org.eclipse.jgit.internal.storage.pack.DeltaTask task : tb.tasks)
	...
	private void writeDeltaObjectDeflate(PackOutputStream out,
		...
		org.eclipse.jgit.internal.storage.pack.DeltaCache.Ref ref = otp.popCachedDelta();
		if (ref != null) {
			byte[] zbuf = ref.get();
			if (zbuf != null) {
	...
	private void findObjectsToPack(@NonNull ProgressMonitor countingMonitor,
		...
		if (thin && !baseTrees.isEmpty()) {
			org.eclipse.jgit.internal.storage.pack.BaseSearch bases = new org.eclipse.jgit.internal.storage.pack.BaseSearch(countingMonitor, baseTrees, //
	...
	public boolean prepareBitmapIndex(ProgressMonitor pm) throws IOException {
		...
		org.eclipse.jgit.internal.storage.pack.PackWriterBitmapPreparer bitmapPreparer = new org.eclipse.jgit.internal.storage.pack.PackWriterBitmapPreparer(

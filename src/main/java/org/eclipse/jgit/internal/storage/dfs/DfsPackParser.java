...
public class DfsPackParser extends PackParser {
	...
	private org.eclipse.jgit.internal.storage.dfs.DfsBlock readBlock;
	...
	private void buffer(byte[] raw, int pos, int len) throws IOException {
		while (0 < len) {
			...
			if (n == 0) {
				org.eclipse.jgit.internal.storage.dfs.DfsBlock v = flushBlock();
				currBuf = new byte[blockSize];
				currEnd = 0;
				currPos += v.size();
				continue;
	...
	private org.eclipse.jgit.internal.storage.dfs.DfsBlock flushBlock() throws IOException {
		if (isEmptyPack)
		...
		org.eclipse.jgit.internal.storage.dfs.DfsBlock v = new org.eclipse.jgit.internal.storage.dfs.DfsBlock(packKey, currPos, buf);
		readBlock = v;
		blockCache.put(v);
		return v;
	}
		...
		if (readBlock == null || !readBlock.contains(packKey, readPos)) {
			...
			if (readBlock == null) {
				...
				if (read(start, buf, 0, size) != size)
					throw new EOFException();
				readBlock = new org.eclipse.jgit.internal.storage.dfs.DfsBlock(packKey, start, buf);

 ...
 * <p>
 * See the base {@link ObjectReader} documentation for
...
public class DfsReader extends ObjectReader implements ObjectReuseAsIs {
	...
	private org.eclipse.jgit.internal.storage.dfs.DfsBlock block;
	private org.eclipse.jgit.internal.storage.dfs.DeltaBaseCache baseCache;
	...
	org.eclipse.jgit.internal.storage.dfs.DeltaBaseCache getDeltaBaseCache() {
		if (baseCache == null)
			baseCache = new org.eclipse.jgit.internal.storage.dfs.DeltaBaseCache(this);
		return baseCache;
	}
			...
			public ObjectLoader open() throws IOException {
				if (cur.pack == null)
					throw new MissingObjectException(cur.id,
							JGitText.get().unknownObjectType2);
				return cur.pack.load(org.eclipse.jgit.internal.storage.dfs.DfsReader.this, cur.offset);
			}
			...
			public boolean next() throws MissingObjectException, IOException {
				if (idItr.hasNext()) {
					...
					if (cur.pack == null)
						throw new MissingObjectException(cur.id,
								JGitText.get().unknownObjectType2);
					sz = cur.pack.getObjectSize(org.eclipse.jgit.internal.storage.dfs.DfsReader.this, cur.offset);
					return true;
				} else if (findAllError != null) {
					throw findAllError;
				} else {
	...
	org.eclipse.jgit.internal.storage.dfs.DfsBlock quickCopy(DfsPackFile p, long pos, long cnt)
			throws IOException {
		pin(p, pos);

...
public final class FirstCommand {
	 ...
	 */
	@NonNull
	public static org.eclipse.jgit.internal.transport.parser.FirstCommand fromLine(String line) {
		int nul = line.indexOf('\0');
		if (nul < 0) {
			return new org.eclipse.jgit.internal.transport.parser.FirstCommand(line, emptySet());
		}
		Set<String> opts =
				asList(line.substring(nul + 1).split(" ")) //$NON-NLS-1$
					.stream()
					.collect(toSet());
		return new org.eclipse.jgit.internal.transport.parser.FirstCommand(line.substring(0, nul), unmodifiableSet(opts));
	}

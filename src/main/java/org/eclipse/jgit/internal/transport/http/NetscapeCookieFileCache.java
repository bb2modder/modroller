...
public class NetscapeCookieFileCache {
	...
	private static org.eclipse.jgit.internal.transport.http.NetscapeCookieFileCache instance;
	 ...
	 *         created)
	 */
	public static org.eclipse.jgit.internal.transport.http.NetscapeCookieFileCache getInstance(HttpConfig config) {
		if (instance == null) {
			return new org.eclipse.jgit.internal.transport.http.NetscapeCookieFileCache(config);
		}
		return instance;
	}
	...
	public NetscapeCookieFile getEntry(Path path) {
		if (!cookieFileMap.containsKey(path)) {
			synchronized (org.eclipse.jgit.internal.transport.http.NetscapeCookieFileCache.class) {
				if (!cookieFileMap.containsKey(path)) {

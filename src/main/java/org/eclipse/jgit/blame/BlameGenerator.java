 ...
 * {@link BlameResult#create(org.eclipse.jgit.blame.BlameGenerator)} and
...
public class BlameGenerator implements AutoCloseable {
	...
	private org.eclipse.jgit.blame.Candidate queue;

	/** Number of lines that still need to be discovered. */
	private int remaining;

	/** Blame is currently assigned to this source. */
	private org.eclipse.jgit.blame.Candidate outCandidate;
	private org.eclipse.jgit.blame.Region outRegion;
	...
	private void initRevPool(boolean reverse) {
		...
		if (reverse)
			revPool = new org.eclipse.jgit.blame.ReverseWalk(getRepository());
		else
	 ...
	 * @return {@code this}
	 */
	public org.eclipse.jgit.blame.BlameGenerator setDiffAlgorithm(DiffAlgorithm algorithm) {
	 ...
	 * @return {@code this}
	 */
	public org.eclipse.jgit.blame.BlameGenerator setTextComparator(RawTextComparator comparator) {
	 ...
	 * @return {@code this}
	 */
	public org.eclipse.jgit.blame.BlameGenerator setFollowFileRenames(boolean follow) {
		if (follow)
	 ...
	 */
	public org.eclipse.jgit.blame.BlameGenerator push(String description, byte[] contents)
			throws IOException {
	 ...
	 *             the repository cannot be read.
	 */
	public org.eclipse.jgit.blame.BlameGenerator push(String description, RawText contents)
			throws IOException {
		if (description == null)
		...
		c.regionList = new org.eclipse.jgit.blame.Region(0, 0, contents.size());
	 ...
	 * @since 5.6
	 */
	public org.eclipse.jgit.blame.BlameGenerator prepareHead() throws NoHeadException, IOException {
		...
		try (TreeWalk walk = new TreeWalk(repo)) {
			...
			if (indexEntry.getStage() == DirCacheEntry.STAGE_0) {
				...
				HeadCandidate c = new HeadCandidate(getRepository(), resultPath,
						getHeads(repo, head));
				c.sourceText = inTree;
				c.regionList = new org.eclipse.jgit.blame.Region(0, 0, inTree.size());
	 ...
	 *             the repository cannot be read.
	 */
	public org.eclipse.jgit.blame.BlameGenerator push(String description, AnyObjectId id)
			throws IOException {
		...
		if (ldr.getType() == OBJ_BLOB) {
			...
			c.regionList = new org.eclipse.jgit.blame.Region(0, 0, c.sourceText.size());
		...
		org.eclipse.jgit.blame.Candidate c = new org.eclipse.jgit.blame.Candidate(getRepository(), commit, resultPath);
	 ...
	 *             the repository cannot be read.
	 */
	public org.eclipse.jgit.blame.BlameGenerator reverse(AnyObjectId start, AnyObjectId end)
			throws IOException {
	 ...
	 *             the repository cannot be read.
	 */
	public org.eclipse.jgit.blame.BlameGenerator reverse(AnyObjectId start,
	...
	public boolean next() throws IOException {
		...
		if (outRegion != null) {
			org.eclipse.jgit.blame.Region r = outRegion;
			remaining -= r.length;
			if (r.next != null) {
				outRegion = r.next;
				return true;
		...
		for (;;) {
			org.eclipse.jgit.blame.Candidate n = pop();
			if (n == null)
				return done();
	...
	private boolean result(org.eclipse.jgit.blame.Candidate n) throws IOException {
	...
	private boolean reverseResult(org.eclipse.jgit.blame.Candidate parent, org.eclipse.jgit.blame.Candidate source)
			throws IOException {
		...
		org.eclipse.jgit.blame.Candidate res = parent.copy(parent.sourceCommit);
		res.regionList = source.regionList;
		return result(res);
	}

	private org.eclipse.jgit.blame.Candidate pop() {
		org.eclipse.jgit.blame.Candidate n = queue;
		if (n != null) {
			queue = n.queueNext;
			n.queueNext = null;
		}
	...
	private void push(BlobCandidate toInsert) {
		org.eclipse.jgit.blame.Candidate c = queue;
		if (c != null) {
	...
	private void push(org.eclipse.jgit.blame.Candidate toInsert) {
		if (toInsert.has(SEEN)) {
			...
			// region list into it.
			for (org.eclipse.jgit.blame.Candidate p = queue; p != null; p = p.queueNext) {
		...
		for (org.eclipse.jgit.blame.Candidate p = n;; p = n) {
			n = p.queueNext;
			if (n == null || time >= n.getTime()) {
				toInsert.queueNext = n;
	...
	private boolean processOne(org.eclipse.jgit.blame.Candidate n) throws IOException {
		...
		org.eclipse.jgit.blame.Candidate next = n.create(getRepository(), parent,
	...
	private boolean blameEntireRegionOnParent(org.eclipse.jgit.blame.Candidate n, RevCommit parent) {
	...
	private boolean splitBlameWithParent(org.eclipse.jgit.blame.Candidate n, RevCommit parent)
			throws IOException {
		org.eclipse.jgit.blame.Candidate next = n.create(getRepository(), parent, n.sourcePath);
		next.sourceBlob = idBuf.toObjectId();
		next.loadText(reader);
		return split(next, n);
	}

	private boolean split(org.eclipse.jgit.blame.Candidate parent, org.eclipse.jgit.blame.Candidate source)
			throws IOException {
	...
	private boolean processMerge(org.eclipse.jgit.blame.Candidate n) throws IOException {
		int pCnt = n.getParentCount();
		...
		org.eclipse.jgit.blame.Candidate[] parents = new org.eclipse.jgit.blame.Candidate[pCnt];
		for (int pIdx = 0; pIdx < pCnt; pIdx++) {
			RevCommit parent = n.getParent(pIdx);

			org.eclipse.jgit.blame.Candidate p;
			...
			if (p.regionList != null) {
				...
				// for deletions.
				if (n instanceof ReverseCandidate) {
					org.eclipse.jgit.blame.Region r = p.regionList;
					p.regionList = n.regionList;
					n.regionList = r;
				}
		...
		if (n instanceof ReverseCandidate) {
			...
			org.eclipse.jgit.blame.Candidate resultHead = null;
			org.eclipse.jgit.blame.Candidate resultTail = null;

			for (int pIdx = 0; pIdx < pCnt; pIdx++) {
				org.eclipse.jgit.blame.Candidate p = parents[pIdx];
				if (p == null)
					continue;

				if (p.regionList != null) {
					org.eclipse.jgit.blame.Candidate r = p.copy(p.sourceCommit);
					if (resultTail != null) {

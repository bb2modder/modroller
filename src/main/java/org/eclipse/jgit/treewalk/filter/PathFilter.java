...
public class PathFilter extends TreeFilter {
	 ...
	 */
	public static org.eclipse.jgit.treewalk.filter.PathFilter create(String path) {
		while (path.endsWith("/")) //$NON-NLS-1$
			path = path.substring(0, path.length() - 1);
		if (path.length() == 0)
			throw new IllegalArgumentException(JGitText.get().emptyPathNotPermitted);
		return new org.eclipse.jgit.treewalk.filter.PathFilter(path);
	}
	...
	/** {@inheritDoc} */
	@Override
	public org.eclipse.jgit.treewalk.filter.PathFilter clone() {
		return this;
	}

 ...
 * threads. Equality of {@link RevCommit}s from two
 * different RevWalk instances is never true, even if their
 * {@link ObjectId}s are equal (and thus they describe the
 * same commit).
 * <p>
...
public class RevWalk implements Iterable<RevCommit>, AutoCloseable {
	...
	private RevWalk(ObjectReader or, boolean closeReader) {
		...
		pending = new org.eclipse.jgit.revwalk.StartGenerator(this);
	...
	public ReachabilityChecker createReachabilityChecker() throws IOException {
		...
		return new org.eclipse.jgit.revwalk.PedestrianReachabilityChecker(true, this);
	}
	 ...
	 * {@link RevSort#TOPO} must take precedence over
	 * {@link RevSort#COMMIT_TIME_DESC}, otherwise it
	 ...
	 * <p>
	 * If null or {@link TreeFilter#ALL} the
	 * path limiter is removed. Commits will not be simplified.
	 * <p>
	 * If non-null and not
	 * {@link TreeFilter#ALL} then the tree
	 ...
	 * {@link TreeFilter#ANY_DIFF} along with
	 ...
	 * null and non-{@link TreeFilter#ALL}
		...
		return new AsyncRevObjectQueue() {
			...
			public RevObject next() throws MissingObjectException,
				...
				if (r == null)
					...
					((RevCommit) r).parseCanonical(org.eclipse.jgit.revwalk.RevWalk.this, raw);
				} else if (r instanceof RevTag) {
					byte[] raw = ldr.getCachedBytes();
					((RevTag) r).parseCanonical(org.eclipse.jgit.revwalk.RevWalk.this, raw);
				} else
					r.flags |= PARSED;
				return r;
			}
	...
	private boolean isNotStarted() {
		return pending instanceof org.eclipse.jgit.revwalk.StartGenerator;
	}

	/**
	 * Create and return an {@link ObjectWalk} using
	 * the same objects.
	 * <p>
	...
	public ObjectWalk toObjectWalkWithSameObjects() {
		...
		org.eclipse.jgit.revwalk.RevWalk rw = ow;
		rw.objects = objects;
	 ...
	 * shallow commit, since {@link RevCommit#parseCanonical(org.eclipse.jgit.revwalk.RevWalk, byte[])}
	 * calls this method before its callers add the new commit to the
	 * {@link org.eclipse.jgit.revwalk.RevWalk#objects} map. That means a call from this method to
	 * {@link #lookupCommit(AnyObjectId)} fails to find that commit and creates
	 * a new one, which is promptly discarded.
	 * <p>
	 * To avoid that, {@link RevCommit#parseCanonical(org.eclipse.jgit.revwalk.RevWalk, byte[])} passes

...
public class RevCommit extends RevObject {
	 ...
	 */
	public static org.eclipse.jgit.revwalk.RevCommit parse(byte[] raw) {
		try {
	 ...
	 */
	public static org.eclipse.jgit.revwalk.RevCommit parse(RevWalk rw, byte[] raw) throws IOException {
		try (ObjectInserter.Formatter fmt = new ObjectInserter.Formatter()) {
			org.eclipse.jgit.revwalk.RevCommit r = rw.lookupCommit(fmt.idFor(Constants.OBJ_COMMIT, raw));
	...
	void parseCanonical(RevWalk walk, byte[] raw) throws IOException {
		...
		int ptr = 46;
		if (parents == null) {
			org.eclipse.jgit.revwalk.RevCommit[] pList = new org.eclipse.jgit.revwalk.RevCommit[1];
			int nParents = 0;
			for (;;) {
				if (raw[ptr] != 'p') {
					break;
				...
				final org.eclipse.jgit.revwalk.RevCommit p = walk.lookupCommit(idBuffer);
				switch (nParents) {
				case 0:
					pList[nParents++] = p;
					break;
				case 1:
					pList = new org.eclipse.jgit.revwalk.RevCommit[] { pList[0], p };
					nParents = 2;
					break;
				default:
					if (pList.length <= nParents) {
						org.eclipse.jgit.revwalk.RevCommit[] old = pList;
						pList = new org.eclipse.jgit.revwalk.RevCommit[pList.length + 32];
			...
			if (nParents != pList.length) {
				org.eclipse.jgit.revwalk.RevCommit[] old = pList;
				pList = new org.eclipse.jgit.revwalk.RevCommit[nParents];
				System.arraycopy(old, 0, pList, 0, nParents);
			}
			parents = pList;
		}
	...
	static void carryFlags(org.eclipse.jgit.revwalk.RevCommit c, int carry) {
		FIFORevQueue q = carryFlags1(c, carry, 0);
		if (q != null)
			slowCarryFlags(q, carry);
	}

	private static FIFORevQueue carryFlags1(org.eclipse.jgit.revwalk.RevCommit c, int carry, int depth) {
		for(;;) {
			org.eclipse.jgit.revwalk.RevCommit[] pList = c.parents;
			...
			if (pList.length != 1) {
				...
				for (int i = 1; i < pList.length; i++) {
					org.eclipse.jgit.revwalk.RevCommit p = pList[i];
	...
	private static FIFORevQueue defer(org.eclipse.jgit.revwalk.RevCommit c) {
	...
	private static FIFORevQueue defer(FIFORevQueue q, int carry,
	                                  org.eclipse.jgit.revwalk.RevCommit[] pList, int i) {
	...
	private static void slowCarryFlags(FIFORevQueue q, int carry) {
		...
		for (org.eclipse.jgit.revwalk.RevCommit c; (c = q.next()) != null;) {
			for (org.eclipse.jgit.revwalk.RevCommit p : c.parents)
				carryOneStep(q, carry, p);
		}
	}

	private static void carryOneStep(FIFORevQueue q, int carry, org.eclipse.jgit.revwalk.RevCommit c) {
		if ((c.flags & carry) != carry) {
			c.flags |= carry;
			if (c.parents != null)
				q.add(c);
	 ...
	 */
	public final org.eclipse.jgit.revwalk.RevCommit getParent(int nth) {
		return parents[nth];
	}

 ...
 */
class StartGenerator extends org.eclipse.jgit.revwalk.Generator {
	...
	RevCommit next() throws MissingObjectException,
		...
		org.eclipse.jgit.revwalk.Generator g;

		final RevWalk w = walker;
		RevFilter rf = w.getRevFilter();
		final TreeFilter tf = w.getTreeFilter();
		org.eclipse.jgit.revwalk.AbstractRevQueue q = walker.queue;
		...
		if (rf == RevFilter.MERGE_BASE) {
			...
			final org.eclipse.jgit.revwalk.MergeBaseGenerator mbg = new org.eclipse.jgit.revwalk.MergeBaseGenerator(w);
			walker.pending = mbg;
			walker.queue = org.eclipse.jgit.revwalk.AbstractRevQueue.EMPTY_QUEUE;
			mbg.init(q);
			return mbg.next();
		}
		...
		if ((g.outputType() & NEEDS_REWRITE) != 0) {
			...
			g = new org.eclipse.jgit.revwalk.RewriteGenerator(g);
		}
		...
		if (walker.hasRevSort(RevSort.TOPO)
			...
			g = new org.eclipse.jgit.revwalk.TopoSortGenerator(g);
		...
		if (boundary)
			g = new org.eclipse.jgit.revwalk.BoundaryGenerator(w, g);
			...
			if (pending.peek() != null)
				g = new org.eclipse.jgit.revwalk.DelayRevQueue(g);
			g = new org.eclipse.jgit.revwalk.FixUninterestingGenerator(g);
		}
